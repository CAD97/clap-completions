...
macro_rules! all_tests {
    ($name:literal, $test:ident) => {
        insta::assert_snapshot!(concat!($name, "/basic"), $test!(basic_command));
        insta::assert_snapshot!(concat!($name, "/feature-sample"), $test!(feature_sample_command));
        insta::assert_snapshot!(concat!($name, "/special-commands"), $test!(special_commands_command));
        insta::assert_snapshot!(concat!($name, "/quoting"), $test!(quoting_command));
        insta::assert_snapshot!(concat!($name, "/aliases"), $test!(aliases_command));
        insta::assert_snapshot!(concat!($name, "/sub-subcommands"), $test!(sub_subcommands_command));
        insta::assert_snapshot!(concat!($name, "/value-hint"), $test!(value_hint_command));
    };
}
    ...
    macro_rules! test {
        ($f:ident) => {
            &{
                let mut app = $f("my-app");
                app.build();
                nu::Completions::new(&app).to_string()
            }
        };
    }
